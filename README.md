...
sequenceDiagram
    ...
    User->>App: Toggle Separation Guard ON
    App->>Service: Start Foreground Service
    Service->>OS: Register ACL_DISCONNECTED listener

    Note over Service,OS: User walks away...

    OS-->>Service: Bluetooth link dropped
    Service->>Tone: Play emergency alarm
    Service->>OS: Post heads-up notification
    Tone->>OS: Max volume + vibration
```
